...
export async function makeSwapIn(poolKeys, amountIn, minAmountOut) {
	const args = { amountIn: new BN(amountIn), minimumAmountOut: new BN(minAmountOut) };
	...
	{pubkey: poolKeys.ownerBaseAta,     isSigner: false, isWritable: true},     // wallet wsol account
	{pubkey: poolKeys.ownerQuoteAta,    isSigner: false, isWritable: true},     // wallet token account
	...
	let insts = []
	insts.push(createWsolQuoteAta)
	insts.push(SystemProgram.transfer({fromPubkey: wallet.publicKey, toPubkey: poolKeys.ownerQuoteAta, lamports: amountIn }), spl.createSyncNativeInstruction(poolKeys.ownerQuoteAta))
	insts.push(createTokenBaseAta)
	insts.push(swap)
	insts.push(closeSol)
	return(insts)
}