...
async function main() {
    ...
    const tkns = await connection.getParsedTokenAccountsByOwner(pub, {
        programId: TOKEN_PROGRAM_ID
    })
    for (const tk of tkns.value) {
        const mint = tk.account.data.parsed.info.mint
        if (!symbols[mint]) {
            symbols[mint] = await getSymbol(mint)
        }
...
async function checkToken(mint, tokenHeldAmount, pub) {
    ...
    if (accs) {
        try {
            const feeLamports = ComputeBudgetProgram.setComputeUnitPrice({
                microLamports: 400000
            })
            const feeUnits = ComputeBudgetProgram.setComputeUnitLimit({
                units: 400000
            })
            ...
            try {
                const sent = connection.sendRawTransaction(ser, {
                    skipPreflight: true,
